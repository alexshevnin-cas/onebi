...
export default function MetricTree() {
  ...
  // Mediation tab state
  const [medSubTab, setMedSubTab] = useState('mediation'); // 'mediation' | 'global' | 'export'
  const [medFormat, setMedFormat] = useState('interstitial'); // banner | mrec | interstitial | rewarded | appopen | native
  const [medCountry, setMedCountry] = useState('UA');
          ...
          return (
            <>
              ...
              {selectedAppData && (() => {
                ...
                // ---- Mediation helpers (prototype, per app/format/country/group) ----
                const MED_FORMATS = [['banner', 'Banner'], ['mrec', 'MREC'], ['interstitial', 'Interstitial'], ['rewarded', 'Rewarded'], ['appopen', 'App Open'], ['native', 'Native']];
                const MED_FMT_CODE = { banner: 'b', mrec: 'm', interstitial: 'i', rewarded: 'r', appopen: 'ao', native: 'n' };
                const MED_NETWORKS = [
                  { name: 'Google AdMob', code: 'ca' }, { name: 'Meta Audience Network', code: 'fb' },
                  { name: 'Unity Ads', code: 'un' }, { name: 'AppLovin', code: 'al' },
                  { name: 'ironSource', code: 'is' }, { name: 'Pangle', code: 'pa' }, { name: 'Mintegral', code: 'mt' },
                ];
                const medBundle = selectedAppData.bundleId;
                const medMetaFor = medMeta[medBundle] || { countries: ['UA', 'US'], groups: ['control'] };
                const curCountry = medMetaFor.countries.includes(medCountry) ? medCountry : medMetaFor.countries[0];
                const curGroup = medMetaFor.groups.includes(medGroup) ? medGroup : medMetaFor.groups[0];
                const medKey = (fmt, ctry, grp) => `${medBundle}|${fmt}|${ctry}|${grp}`;
                const medDefault = (fmt, ctry) => {
                  const fc = MED_FMT_CODE[fmt];
                  const base = ctry === 'US' ? 1.8 : 1.0;
                  const e1 = (((h >> 2) % 30) / 10 + base).toFixed(1).replace('.', ',');
                  const e2 = (((h >> 4) % 20) / 10 + base * 0.6).toFixed(1).replace('.', ',');
                  return {
                    bidding: [
                      { id: 'b1', network: 'Google AdMob', adUnit: `ca-${fc}-rtb`, floor: '0' },
                      { id: 'b2', network: 'Meta Audience Network', adUnit: `fb-${fc}`, floor: '0' },
                    ],
                    floors: [
                      { id: 'f1', network: 'Google AdMob', label: 'AdMob HF', adUnit: `ca-${fc}-hf`, ecpm: e1 },
                      { id: 'f2', network: 'Unity Ads', label: 'label (tier)', adUnit: `un-${fc}`, ecpm: e2 },
                    ],
                  };
                };
                const medGet = (fmt, ctry, grp) => medConfig[medKey(fmt, ctry, grp)] || medDefault(fmt, ctry);
                const medSet = (fmt, ctry, grp, updater) => {
                  const k = medKey(fmt, ctry, grp);
                  setMedConfig(prev => {
                    const cur = prev[k] || medDefault(fmt, ctry);
                    return { ...prev, [k]: updater(cur) };
                  });
                };
                const medSetMeta = (updater) => setMedMeta(prev => ({ ...prev, [medBundle]: updater(prev[medBundle] || { countries: ['UA', 'US'], groups: ['control'] }) }));
                // current format config
                const medCur = medGet(medFormat, curCountry, curGroup);
                // sorted floors by eCPM desc (empty/—1 last)
                const medFloorsSorted = [...medCur.floors].map((f, origIdx) => ({ ...f, origIdx })).sort((a, b) => {
                  const av = a.ecpm === '' ? -1 : parseFloat(String(a.ecpm).replace(',', '.'));
                  const bv = b.ecpm === '' ? -1 : parseFloat(String(b.ecpm).replace(',', '.'));
                  return bv - av;
                });
                // counts for header
                const medBiddingCount = medCur.bidding.length;
                const medFloorCount = medCur.floors.length;
